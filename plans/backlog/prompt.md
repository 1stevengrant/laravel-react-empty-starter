...
Complete the task following Laravel conventions:

- Use `php artisan make:*` to generate files (models, controllers, migrations, etc.)
- Follow existing patterns in the codebase — check sibling files first
- Write Pest tests for all new functionality
- Use Data classes for validation and transformation
- Use Actions for business logic
- Use invokable controllers for non-CRUD endpoints
...
- `php artisan test --compact` to run the tests
- `vendor/bin/pint --dirty` to fix code style
...
If the task is complete, close the original GitHub issue using `gh issue close`.

If the task is not complete, leave a comment on the GitHub issue with what was done using `gh issue comment`.
